...
- `levels.ts` — `buildOverworld()` and `buildDelve()` return tilemaps as `TileChar[][]` where chars encode entities: `#` solid, `=` one-way platform, `c` collectible, `C` big collectible, `n` NPC, `p` portal-to-delve, `r` portal-to-overworld. `isSolid` / `isPlat` are the predicates. `DelveLevel` also carries `enemySpawns: EnemySpawn[]`.
...
- `save.ts` — localStorage persistence under `drift:save:*` and `drift:save_manifest`. Per-save data is `{ character, hud, pos, collected[], defeatedEnemies[], delveCleared }`; manifest is the index shown in the load menu.
