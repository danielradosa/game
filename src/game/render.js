...
export function draw(ctx, s, ch) {
  ...
  for (const bp of s.bgPart) {
    ...
    ctx.beginPath(); ctx.arc(bp.x, bp.y, bp.s, 0, Math.PI * 2); ctx.fill();
  }
  ctx.globalAlpha = 1;
  drawTiles(ctx, s);
  drawEntities(ctx, s);
...
function drawSky(ctx, s) {
  ...
  if (lv.theme === 'delve') {
    grad.addColorStop(0, '#1a0a2a'); grad.addColorStop(0.5, '#2a1040'); grad.addColorStop(1, '#0a0518');
  } else {
    grad.addColorStop(0, '#f4b58a'); grad.addColorStop(0.4, '#e88c8a');
    grad.addColorStop(0.75, '#7a6ab0'); grad.addColorStop(1, '#3a4078');
  }
...
function drawParallax(ctx, s) {
  ...
  const range = (offX, baseY, color, amp, freq) => {
    ctx.fillStyle = color; ctx.beginPath(); ctx.moveTo(0, lv.H * T);
    for (let x = 0; x <= lvW; x += 20) {
      const px = x + offX;
      const y = baseY + Math.sin(px * freq) * amp + Math.cos(px * freq * 2.3) * amp * 0.5;
      ctx.lineTo(x, y);
    }
    ctx.lineTo(lvW, lv.H * T); ctx.closePath(); ctx.fill();
  };
...
function drawTiles(ctx, s) {
  ...
  for (let ty = sY; ty <= eY; ty++) for (let tx = sX; tx <= eX; tx++) {
    const c = map[ty][tx], x = tx * T, y = ty * T;
...
function drawSolid(ctx, x, y, map, tx, ty, theme) {
  ...
  if (theme === 'delve') { if (tryDrawSprite(ctx, 'tile_ground_delve', x, y)) return; }
  else { if (isTop && tryDrawSprite(ctx, 'tile_grass', x, y)) return; if (!isTop && tryDrawSprite(ctx, 'tile_ground', x, y)) return; }
  if (theme === 'delve') {
    ctx.fillStyle = '#3a2a55'; ctx.fillRect(x, y, T, T);
    ctx.fillStyle = '#2a1a40'; ctx.fillRect(x, y, 2, T); ctx.fillRect(x, y + T - 2, T, 2);
    if (isTop) { ctx.fillStyle = '#5a3a8a'; ctx.fillRect(x, y, T, 4); }
    if ((tx * 7 + ty * 13) % 11 === 0) {
    ...
    earth.addColorStop(0, '#6b4a32'); earth.addColorStop(1, '#3a2818');
    ctx.fillStyle = earth; ctx.fillRect(x, y, T, T);
    ...
    if (isTop) {
      ...
      grass.addColorStop(0, '#a8d05a'); grass.addColorStop(1, '#5a8030');
      ctx.fillStyle = grass; ctx.fillRect(x, y, T, 8);
...
function drawPlatform(ctx, x, y, theme) {
  if (theme === 'delve') { if (tryDrawSprite(ctx, 'tile_platform_delve', x, y)) return; }
  else { if (tryDrawSprite(ctx, 'tile_platform', x, y)) return; }
  if (theme === 'delve') {
    ctx.fillStyle = '#5a3a8a'; ctx.fillRect(x, y, T, 6);
    ctx.fillStyle = '#3a2a55'; ctx.fillRect(x, y + 6, T, 4);
    ctx.fillStyle = 'rgba(200,150,255,0.5)'; ctx.fillRect(x, y, T, 1);
  } else {
    const g = ctx.createLinearGradient(x, y, x, y + 12);
    g.addColorStop(0, '#9a6f48'); g.addColorStop(0.5, '#7a4f30'); g.addColorStop(1, '#4a2818');
    ctx.fillStyle = g; ctx.fillRect(x, y, T, 12);
    ctx.fillStyle = 'rgba(0,0,0,0.2)'; ctx.fillRect(x, y + 11, T, 1);
    ctx.fillStyle = 'rgba(255,220,170,0.3)'; ctx.fillRect(x + 2, y + 1, T - 4, 1);
  }
}
...
function drawEntities(ctx, s) {
  ...
  for (let ty = 0; ty < lv.H; ty++) for (let tx = 0; tx < lv.W; tx++) {
    const c = map[ty][tx], x = tx * T, y = ty * T, key = tx + ',' + ty;
    if (c === 'c' && !s.collected.has(key)) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, -8); ctx.lineTo(6, -2); ctx.lineTo(4, 7); ctx.lineTo(-4, 7); ctx.lineTo(-6, -2); ctx.closePath(); ctx.fill();
      ctx.fillStyle = '#fff8c0';
      ctx.beginPath(); ctx.moveTo(0, -8); ctx.lineTo(6, -2); ctx.lineTo(2, 0); ctx.closePath(); ctx.fill();
      ctx.restore();
      ...
      g.addColorStop(0, 'rgba(200,150,255,0.7)'); g.addColorStop(1, 'rgba(200,150,255,0)');
      ctx.fillStyle = g; ctx.fillRect(-50, -50, 100, 100);
      ctx.fillStyle = '#c4a0e8';
      ctx.beginPath();
      ctx.moveTo(0, -16); ctx.lineTo(12, -4); ctx.lineTo(8, 14); ctx.lineTo(-8, 14); ctx.lineTo(-12, -4); ctx.closePath(); ctx.fill();
      ctx.fillStyle = '#fff';
      ctx.beginPath(); ctx.moveTo(0, -16); ctx.lineTo(12, -4); ctx.lineTo(4, 0); ctx.closePath(); ctx.fill();
      ctx.restore();
    } else if (c === 'p') {
      ...
      if (!tryDrawSprite(ctx, 'portal_delve', cx, by)) {
        ...
        for (let i = 3; i >= 0; i--) {
          ...
          ctx.beginPath(); ctx.ellipse(cx, cy - 8, r * 0.7, r, 0, 0, Math.PI * 2); ctx.fill();
        }
        ctx.fillStyle = '#1a0a2a';
        ctx.beginPath(); ctx.ellipse(cx, cy - 8, 14, 22, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = 'rgba(255,200,255,0.6)'; ctx.fillRect(cx - 1, cy - 25, 2, 35);
        ctx.restore();
      }
      ctx.fillStyle = 'rgba(255,255,255,0.85)'; ctx.font = '12px sans-serif'; ctx.textAlign = 'center';
      ...
      if (!tryDrawSprite(ctx, 'portal_return', cx, by)) {
        ...
        for (let i = 3; i >= 0; i--) {
          ...
          ctx.beginPath(); ctx.ellipse(cx, cy - 6, r * 0.7, r, 0, 0, Math.PI * 2); ctx.fill();
        }
        ctx.fillStyle = '#3a1a0a';
        ctx.beginPath(); ctx.ellipse(cx, cy - 6, 12, 18, 0, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
      }
      ctx.fillStyle = 'rgba(255,255,255,0.85)'; ctx.font = '12px sans-serif'; ctx.textAlign = 'center';
      ...
      ctx.fillStyle = '#3a2030'; ctx.beginPath(); ctx.ellipse(cx, by + 2, 10, 3, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#5a4480'; ctx.fillRect(cx - 8, by - 22, 16, 18);
      ctx.fillStyle = '#e8c1a0'; ctx.beginPath(); ctx.arc(cx, by - 28, 7, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = '#3a2820'; ctx.fillRect(cx - 7, by - 33, 14, 6);
    }
  }
}
...
function drawPlayer(ctx, s, ch) {
  ...
  if (isReady(SPRITES.player)) {
    ctx.save(); ctx.translate(Math.round(cx), Math.round(by)); ctx.scale(p.facing, 1);
    const spec = ASSET_SIZES.player;
    ctx.drawImage(SPRITES.player, -spec.w / 2, -spec.h, spec.w, spec.h);
    ctx.restore(); return;
  }
  const sq = p.squash;
  const bodyH = PH * sq, bodyW = PW * (2 - sq);
  const moving = Math.abs(p.vx) > 0.5 && p.onGround;
  const swing = Math.sin(p.anim * 4) * (moving ? 1 : 0);
  const inAir = !p.onGround;
  ctx.save(); ctx.translate(Math.round(cx), Math.round(by)); ctx.scale(p.facing, 1);
  ctx.fillStyle = 'rgba(0,0,0,0.3)';
  ctx.beginPath(); ctx.ellipse(0, 2, 12 * (p.onGround ? 1 : 0.6), 3, 0, 0, Math.PI * 2); ctx.fill();
  if (p.dashFrames > 0) {
    ctx.fillStyle = ch.accent; ctx.globalAlpha = 0.4;
  ...
  tg.addColorStop(0, shade(ch.shirt, -20)); tg.addColorStop(0.5, ch.shirt); tg.addColorStop(1, shade(ch.shirt, -10));
  ctx.fillStyle = tg; ctx.fillRect(-bodyW / 2, -bodyH * 0.78, bodyW, bodyH * 0.4);
  ctx.fillStyle = ch.accent; ctx.fillRect(-bodyW / 2, -bodyH * 0.55, bodyW, 2);
  ...
  ctx.fillStyle = ch.skin; ctx.beginPath(); ctx.arc(0, -bodyH * 0.86, 8, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = ch.hair;
  if (ch.hairStyle === 'short') {
    ctx.beginPath(); ctx.arc(0, -bodyH * 0.92, 8, Math.PI, 0); ctx.fill();
  } else if (ch.hairStyle === 'med') {
    ctx.beginPath(); ctx.arc(0, -bodyH * 0.92, 9, Math.PI, 0); ctx.fill();
    ctx.fillRect(-9, -bodyH * 0.92, 4, 8); ctx.fillRect(5, -bodyH * 0.92, 4, 6);
  } else {
    ctx.beginPath(); ctx.arc(0, -bodyH * 0.92, 9, Math.PI, 0); ctx.fill();
    ctx.fillRect(-9, -bodyH * 0.93, 4, 18); ctx.fillRect(5, -bodyH * 0.93, 4, 18);
  }
  ctx.fillStyle = '#1a1a1a';
