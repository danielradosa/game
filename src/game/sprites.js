...
export const SPRITES = {
  player: null, tile_ground: null, tile_grass: null, tile_platform: null,
  collectible: null, cache: null, portal_delve: null, portal_return: null,
  npc: null, tile_ground_delve: null, tile_platform_delve: null,
};
export const isReady = (s) => s && s.complete && s.naturalWidth > 0;
export function tryDrawSprite(ctx, name, x, y) {
  const s = SPRITES[name]; if (!isReady(s)) return false;
  const spec = ASSET_SIZES[name];
  let dx = x, dy = y;
  if (spec.anchor === 'center')             { dx = x - spec.w / 2; dy = y - spec.h / 2; }
  else if (spec.anchor === 'bottom-center') { dx = x - spec.w / 2; dy = y - spec.h; }
