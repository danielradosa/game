...
export default function App() {
  ...
  const [character, setCharacter] = useState({
    name: 'Wren', skin: SKINS[1], hair: HAIRS[1], hairStyle: 'med',
    shirt: SHIRTS[0], pants: PANTS[0], accent: ACCS[0],
  });
  const [hud, setHud] = useState({ level: 1, xp: 0, materials: 0, discovered: [], achievements: [], inDelve: false });
  ...
  const inputRef = useRef({
    ...
    jump: false, jumpEdge: false, dash: false, dashEdge: false,
    interact: false, interactEdge: false,
  });
  const charRef = useRef(character); charRef.current = character;
  const hudRef = useRef(hud); hudRef.current = hud;
  const pausedRef = useRef(paused); pausedRef.current = paused;

  useEffect(() => { setMutedAudio(muted); }, [muted]);
  useEffect(() => { setManifest(fetchManifest()); }, []);
  ...
  const grantXP = useCallback((amt, label) => {
    setHud(h => {
      let xp = h.xp + amt, lvl = h.level;
      const ups = [];
      while (xp >= xpFor(lvl)) { xp -= xpFor(lvl); lvl++; ups.push(lvl); }
      if (label) pushNotif('+' + amt + ' XP · ' + label, 'xp');
      ups.forEach(u => { pushNotif('Level ' + u, 'level'); playSnd('level_up'); });
      return { ...h, xp, level: lvl };
    });
  }, [pushNotif]);

  const discover = useCallback((zoneId) => {
  ...
  const transitionToDelve = useCallback(() => {
    ...
    s.current = 'delve'; s.level = s.dl;
    s.p.x = s.dl.spawn.x; s.p.y = s.dl.spawn.y;
    s.p.vx = 0; s.p.vy = 0; s.p.dashFrames = 0; s.p.dashCool = 0;
    s.collected = new Set();
    setHud(h => ({ ...h, inDelve: true }));
    grantAch('a7'); pushNotif('Entered the Delve', 'discovery'); playSnd('portal');
  }, [grantAch, pushNotif]);

  const transitionToOver = useCallback(() => {
    const s = stateRef.current;
    s.current = 'over'; s.level = s.ow;
    s.p.x = 100 * T; s.p.y = (s.ow.ground[100] - 3) * T;
    s.p.vx = 0; s.p.vy = 0; s.p.dashFrames = 0; s.p.dashCool = 0;
    setHud(h => ({ ...h, inDelve: false }));
    pushNotif('Returned to the surface', 'discovery'); playSnd('portal');
  }, [pushNotif]);
  ...
  const saveCurrent = useCallback(() => {
    ...
    const data = {
      character: charRef.current, hud: hudRef.current,
      pos: { x: s.p.x, y: s.p.y, scene: s.current },
      collected: Array.from(s.collected),
    };
    const meta = {
      id, name: charRef.current.name, level: hudRef.current.level,
      materials: hudRef.current.materials, discovered: hudRef.current.discovered.length,
  ...
  const startGameFresh = useCallback(() => {
    const ow = buildOverworld(), dl = buildDelve();
    ...
    setNotifs([]); setPaused(false); setShowInv(false); setScene('play');
  }, []);

  const loadGameById = useCallback((id) => {
    const data = getSave(id);
    if (!data) { pushNotif('Load failed', 'xp'); return; }
    const ow = buildOverworld(), dl = buildDelve();
    stateRef.current = makeInitialState(ow, dl, data.pos.x, data.pos.y, data.pos.scene, new Set(data.collected || []));
    setCharacter(data.character);
    setHud({ ...data.hud, inDelve: data.pos.scene === 'delve' });
    setNotifs([]); setPaused(false); setShowInv(false); setScene('play');
  }, [pushNotif]);

  const deleteSaveById = useCallback((id) => {
    deleteSave(id);
  ...
  useEffect(() => {
    ...
    const down = (e) => {
      ...
      if (['arrowup', 'w', ' ', 'z'].includes(k)) { if (!inp.jump) inp.jumpEdge = true; inp.jump = true; }
      if (['shift', 'x', 'k'].includes(k)) { if (!inp.dash) inp.dashEdge = true; inp.dash = true; }
      if (k === 'e' || k === 'enter') { if (!inp.interact) inp.interactEdge = true; inp.interact = true; }
      if (k === 'i' || k === 'tab') { e.preventDefault(); setShowInv(v => !v); }
  ...
  if (scene === 'menu') return <MainMenu manifest={manifest} muted={muted}
  ...
  if (scene === 'loadmenu') return <LoadMenu manifest={manifest} onLoad={loadGameById} onDelete={deleteSaveById} onBack={() => setScene('menu')} />;
  if (scene === 'creator') return <CharacterCreator character={character} setCharacter={setCharacter} onPlay={startGameFresh} onBack={() => setScene('menu')} />;

  const xpPct = (hud.xp / xpFor(hud.level)) * 100;
  return (
    <div className="w-full min-h-screen flex items-center justify-center p-4" style={{ background: '#0a0518' }}>
      <div className="relative" style={{ width: VW, height: VH }}>
        ...
        <div className="absolute top-3 left-3 flex items-center gap-3 pointer-events-none">
          <div className="bg-black/40 backdrop-blur rounded-lg px-3 py-2 text-white text-sm">
            ...
            <div className="text-[10px] text-stone-400 mt-0.5">{hud.xp} / {xpFor(hud.level)} XP</div>
          </div>
          <div className="bg-black/40 backdrop-blur rounded-lg px-3 py-2 text-white text-sm">
          ...
          <div className="bg-black/40 backdrop-blur rounded-lg px-3 py-2 text-white text-sm">
            ...
            <div className="font-bold text-green-200">{hud.discovered.length} / {ZONES.length}</div>
          </div>
          {hud.inDelve && <div className="bg-purple-900/60 backdrop-blur rounded-lg px-3 py-2 text-purple-100 text-xs font-semibold animate-pulse">DELVE</div>}
        </div>
        ...
        <div className="absolute bottom-3 left-3 pointer-events-none text-white/70 text-xs space-y-0.5 bg-black/30 backdrop-blur rounded-lg px-3 py-2">
          <div><b>Move</b> A/D · <b>Jump</b> Space · <b>Dash</b> Shift (8-dir) · <b>Use</b> E</div>
        ...
        <div className="absolute top-20 right-3 flex flex-col gap-2 pointer-events-none">
          {notifs.map(n => (
            <div key={n.id} className={'px-3 py-2 rounded-lg backdrop-blur text-sm font-medium ' + (
              n.kind === 'ach' ? 'bg-yellow-300/90 text-stone-900' :
              n.kind === 'level' ? 'bg-orange-400/90 text-stone-900' :
              n.kind === 'discovery' ? 'bg-emerald-400/90 text-stone-900' :
        ...
        {zoneBanner && performance.now() - zoneBanner.t < 3000 && (
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
            ...
            <div className="text-4xl font-bold text-white" style={{ textShadow: '0 2px 20px rgba(255,180,120,0.8)' }}>{zoneBanner.name}</div>
          </div>
        )}
        {paused && !showInv && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center rounded-lg">
            <div className="text-center space-y-4">
              ...
              <div className="flex flex-col gap-2 items-center">
                <button onClick={() => { playSnd('click'); setPaused(false); }} className="bg-orange-300 text-stone-900 px-8 py-2 rounded-full font-semibold">Resume</button>
                <button onClick={() => { playSnd('click'); saveCurrent(); }} className="bg-emerald-400 text-stone-900 px-8 py-2 rounded-full font-semibold">Save Game</button>
                <button onClick={() => { playSnd('click'); setShowInv(true); }} className="text-stone-300 hover:text-white text-sm">Inventory</button>
                <button onClick={() => setMuted(v => !v)} className="text-stone-300 hover:text-white text-sm">{muted ? '🔇 Sound off' : '🔊 Sound on'}</button>
                <button onClick={() => { playSnd('click'); setPaused(false); setScene('menu'); }} className="text-stone-300 hover:text-white text-sm">Quit to menu</button>
              </div>
            </div>
          </div>
        )}
        {showInv && <InventoryPanel hud={hud} character={character} onClose={() => setShowInv(false)} />}
      </div>
    </div>
  );
}
